...
class Constants(BaseConstants):
    name_in_url = 'csr_3_stage_sjurBot_30' #SH
    ...
    instructions_template = 'csr_sjurBot_30/instruc.html'
    review_template = 'csr_sjurBot_30/review.html'
...
class Group(BaseGroup):
	...
	def A1A2_update(self):
		...
		player = [p for p in self.get_players()][0]

		if player.terminal_choice == 'A1': 
		...
		# 		F_Endow = p.participant.vars['ret_score'] + Constants.endowment_boost
			...
			A_GE = (player.participant.vars['overall_ge_percent'] * player.participant.vars['ret_score']) 
			A_Endow = player.participant.vars['ret_score'] + Constants.endowment_boost
		...
		elif player.terminal_choice == 'A2': 
			pass
	...
	def F1F2_update(self):
		...
		#if F_player.terminal_choice == 'F1': #F2 means pass to A to make decision A3/A4 #SH player chooses F1
		for p in self.get_players():
			# p.participant.vars['end_this_stage_round'] = True #end this round
			A_GE = (p.participant.vars['overall_ge_percent'] * p.participant.vars['ret_score']) 
			A_Endow = p.participant.vars['ret_score'] + Constants.endowment_boost
	...
	def A3A4_update(self):
		...
		elif player.terminal_choice == 'A4':  #A4
			pass
...
class Player(BasePlayer):
	...
	quiz_01_a = models.FloatField(
	    ...
	    min = 0,
	    max = 999,
	    initial=None, #SH autopopulate hack
	...
	def set_round_payoff(self):
		...
		for p in self.group.get_players():
			...
			#	p.postStage_op_group_exchange = str(p.GE_Low_A)
				p.postStage_round_f_points = F_score
	...
	q_degree = models.CharField(
		verbose_name="What is your highest completed education level?",
		choices=["Primary education","Secondary education","Some university education","Undergraduate degree","Graduate degree"],
		doc = ''' degrees ''',
		)

	q_field = models.CharField(
		verbose_name="If you studied at university or vocational school, what field did you study?",
		doc = ''' Field of study? ''',
		)

	q_ybrac = models.CharField(
		verbose_name="What was your household's approximate income last month",
		doc = ''' Approx. household Y? ''',
		)

	q_brtenure = models.CharField(
		verbose_name="How many years have you worked with Blue Ribbon?",
		doc = ''' Blue Ribbon tenure? ''',
		)

	q_brhours = models.CharField(
		verbose_name="Approximately how many hours do you volunteer for Blue Ribbon in a typical week?",
		doc = ''' Blue Ribbon hours per week? ''',
		)
	...
	q_part3_A1strat = models.CharField(
		blank = None,
		verbose_name="In Part 3, how did you decide between A1 and A2?",
		doc = ''' In Part 3, how did you decide between A1 and A2? ''',
		widget=widgets.Textarea)
		
	q_part3_A3strat = models.CharField(
		blank = None,
		verbose_name="In Part 3, how did you decide between A3 and A4?",
		doc = ''' In Part 3, how did you decide between A3 and A4? ''',
		widget=widgets.Textarea)
