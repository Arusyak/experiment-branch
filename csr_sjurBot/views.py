...
class InitWaitPage(WaitPage):
    ...
    def is_displayed(self):
        ...
        if 'overall_ge_percent' not in self.participant.vars: 
            overall_ge_percent = random.choice([0.30,0.40,0.50,0.60])
            self.participant.vars["overall_ge_percent"] = overall_ge_percent
        else: 
            overall_ge_percent = self.participant.vars["overall_ge_percent"]

        if 'final_score' not in self.participant.vars: 
            final_score = random.choice([30,40,50,60])
            self.participant.vars["final_score"] = final_score
        else: 
    ...
    def after_all_players_arrive(self):
        ...
        list_players = [p for p in players]
...
class quiz1(Page):
    ...
    def quiz_01_c_error_message(self, value):
        result = (self.player.GE_Low_A + self.player.GE_Low_F) * self.player.mpcr + (20 - self.player.GE_Low_A)
        if (value != result):
            return 'Incorrect. Feel free to raise your hand to ask for help. '

    def quiz_01_d_error_message(self, value):
        result = (self.player.GE_Low_A + self.player.GE_Low_F) * self.player.mpcr + (20 - self.player.GE_Low_F)
        if (value != result):
            return 'Incorrect. Feel free to raise your hand to ask for help. '

    def quiz_01_e_error_message(self, value):
        F_GE = self.player.GE_Low_F * self.player.F1_F_mult
        GE   = self.player.GE_Low_A + F_GE
...
class quiz2(Page):
    ...
    def quiz_02_c_error_message(self, value):
        ...
        GE = self.player.GE_Low_A + F_GE
        F_IE = 20 -  F_GE
        result = (self.player.GE_Low_A + self.player.GE_Low_F ) * self.player.mpcr + (20 - self.player.GE_Low_A - self.player.boycott_cost)
        if (value != result):
            return 'Incorrect. Feel free to raise your hand to ask for help. '
    ...
    def quiz_02_d_error_message(self, value):
        ...
        GE = self.player.GE_Low_A + F_GE
        F_IE = 20 -  F_GE
        result = (self.player.GE_Low_A + self.player.GE_Low_F) * self.player.mpcr + F_IE
        if (value != result):
            return 'Incorrect. Feel free to raise your hand to ask for help. '
...
class pregame(Page):
    ...
    def vars_for_template(self):
        ...
        #ges_percent_extra_2p = self.participant.vars['overall_ge_percent_list'][:] #SH
        ...
        round_points = (
            self.participant.vars["ret_score"]) #SH TK hack  
...
class Results(Page):
    ...
    def vars_for_template(self):
        ...
        return {
            ...
            'counter_party_round_payoff': self.player.postStage_round_f_points,
...
class FinalResults1(Page):
    ...
    form_model = models.Player
    form_fields = [
        'q_birthMonth','q_birthYear','q_sex','q_languages','q_degree','q_field','q_ybrac','q_brtenure','q_brhours',
        'q_part2strat','q_part3_A1strat','q_part3_A3strat','q_part3_dynamic']
    ...
    def vars_for_template(self):
        ...
        self.session.config['participation_fee'] = c(600).to_real_world_currency(self.session)
        ...
        'point_aed_convert':round(prev_player.participant.vars['final_score_discounter'],2),
        'final_cash':c(self.player.payoff).to_real_world_currency(self.session) + c(600).to_real_world_currency(self.session)
        }
